...
$$ {\mathbf{\overset{\rightarrow}{A}} = A_{x}\mathbf{\hat{i}} + A_{y}\mathbf{\hat{j}} + A_{z}\mathbf{\hat{k}}}\ \text{and}\ {\mathbf{\overset{\rightarrow}{B}} = B_{x}\mathbf{\hat{i}} + B_{y}\mathbf{\hat{j}} + B_{z}\mathbf{\hat{k}}} $$

The dot product can be computed as below:

$$
\begin{array}{cll}
{\mathbf{\overset{\rightarrow}{A}} \cdot \mathbf{\overset{\rightarrow}{B}}} & = & {(A_{x}\mathbf{\hat{i}} + A_{y}\mathbf{\hat{j}} + A_{z}\mathbf{\hat{k}}) \cdot (B_{x}\mathbf{\hat{i}} + B_{y}\mathbf{\hat{j}} + B_{z}\mathbf{\hat{k}})} \\
...
The first dog pulls with force F1=(10.0iˆ−20.4jˆ+2.0kˆ)N, the second dog pulls with force  F2=(−15.0iˆ−6.2kˆ)N, and the third dog pulls with force F3=(5.0iˆ+12.5jˆ)N.

![four vectors](images/three-dogs-pulling-on-a-stick.jpg)

**FIND:**

The angle between forces F1 and F2.
...
$$
\begin{array}{cl}
{\mathbf{\overset{\rightarrow}{F}}_{1} \cdot \mathbf{\overset{\rightarrow}{F}}_{2}} & {= F_{1x}F_{2x} + F_{1y}F_{2y} + F_{1z}F_{2z}} \\
...
$$ \text{cos}\ \varphi = \frac{\mathbf{\overset{\rightarrow}{F}}_{1} \cdot \mathbf{\overset{\rightarrow}{F}}_{2}}{F_{1}F_{2}} $$ 
...
When force F acts on an object and causes the object to move displacement D, we say the force performs work. The amount of work the force does is the dot product F*D.

![four vectors](images/three-dogs-pulling-on-a-stick.jpg)

**FIND:**

If the stick in the image above moves momentarily and gets displaced by vector D =(−7.9jˆ−4.2kˆ)cm, how much work is done by the third dog that exerts F3=(5.0iˆ+12.5jˆ)N?
 
**SOLUTION:**

Compute the dot product of displacement vector D with force vector  F3=(5.0iˆ+12.5jˆ)N, which is the force exerted by the third dog. Let’s use W3 to denote the work done by force F3 on displacement D.

Calculate the dot product of F3 and D:

$$
\begin{array}{cl}
W_{3} & {= \mathbf{\overset{\rightarrow}{F}}_{3} \cdot \mathbf{\overset{\rightarrow}{D}} = F_{3x}D_{x} + F_{3y}D_{y} + F_{3z}D_{z}} \\
...
The SI unit of work is a joule (J) where 1 J = 1  N⋅m. The unit  cm⋅N can be written as 10^−2 m⋅N = 10^−2J, so the solution can be expressed as W3 = −0.9875J ≈ −1.0J.
